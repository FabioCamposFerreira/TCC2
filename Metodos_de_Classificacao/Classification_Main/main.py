...
class MachineLearn:
    def __init__(self, library_index, feature_index, data_base_path, csv_name, csv_features):
    ...
    def step_one(self):
        ...
        for arq in self.data_base:
            ...
            self.images_processed.append([os.path.basename(arq),
    ...
    def step_two(self):
        ...
        for img in self.images_processed:
            ...
            self.images_features.append([int(img[0].split(".")[0]),
                                         feature_extraction.get_features(img[1], self.feature)])
    ...
    def step_three(self):
        ...
        print("\033[92m {}\033[00m".format(self.y_test))
        index=list(range(len(self.images_processed)))
        self.images_processed=np.array(self.images_processed)
        i_train, i_test = train_test_split([index][0], test_size=0.2, random_state=0)
        print("\033[93m {}\033[00m".format(i_test))
        exit(1)
    ...
    def step_four(self):
        ...
        for method in self.methods:
            ...
            for index in range(len(self.images_processed)):
                self.results.append([self.images_processed[index][0], self.y_test[index],
                                    int(class_predictions[index]), end_time])

    def step_five(self):
        """Save the results"""
        self.results = np.array(self.results)
        self.results.sort(axis=0)
...
if __name__ == "__main__":
    ...
    mls += [MachineLearn(library_index=0, feature_index=0,
                         data_base_path="../../Data_Base/Data_Base_Cedulas/",
                         csv_name=str(date.today())+" - Resultados ML1.csv",
                         csv_features=str(date.today())+" - Características ML 1.csv")]
    # Run machine learns
    for ml in mls:
        ml.run()
