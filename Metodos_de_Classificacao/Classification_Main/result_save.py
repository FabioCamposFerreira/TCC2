...
def graphics_generate(files_name: str, images_features: list):
    ...
    for c in classes:
        ...
        # Mean
        plt.plot(np.mean(features[positions], axis=0), label="Nota de"+str(c))
        plt.legend()
        plt.title("Media")
        plt.savefig(files_name.replace("XXX", str(c))+".svg")
    # Median
    plt.plot(np.median(features, axis=0), label="Mediana")
    # Mode
    plt.plot(stats.mode(features, axis=0)[0][0], label="Moda")
