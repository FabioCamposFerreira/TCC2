...
def features_save(csv_name, images_features):
    ...
    with open(csv_name, "w") as csv_file:
        for y, features in images_features:
            row = str(y)+","+str(images_features).replace("[", "",).replace("]", "",)
            csv_file.write(row)
...
def predict_line(result):
    """Construct one line for one result of the classfier
    """
...
def save(csv_name, methods, results, library, feature, data_base_path):
    ...
    write_row(csv_name, row)
    row = "Biblioteca: " + library
    write_row(csv_name, row)
    row = "Caracteristica: " + feature
    write_row(csv_name, row)
    row = "Local das imagens: " + data_base_path
    write_row(csv_name, row)
