...
def segment_chars(plate_img, fixed_width):
    ...
    for label in np.unique(labels):
        ...
        if len(cnts) > 0:
            ...
            cv2.imwrite("contorno2.png", labelMask[y:y + h, x:x + w])
            ...
            # the rules tests
            keepAspectRatio = aspectRatio < 1.1
            keepSolidity = solidity > -0.15
            keepHeight = heightRatio > 0.0 and heightRatio < 0.95
    ...
    if contours:
        ...
        for c in contours:
            ...
            cv2.imwrite("characters.png", temp)
...
class PlateFinder:
    ...
    def find_possible_plates(self, input_img):
        ...
        cv2.imwrite("after_preprocess.png", self.after_preprocess)
        possible_plate_contours = self.extract_contours(self.after_preprocess)
        for cnts in possible_plate_contours:
            x, y, w, h = cv2.boundingRect(cnts)
            new_img = np.array(input_img)
            new_img[y:y + h, x:x + w] = [0, 0, 255]
            cv2.imwrite("contorno.png", new_img)
...
if __name__ == "__main__":
    ...
    cap = cv2.VideoCapture('test_videos/test1.MOV')
    while (cap.isOpened()):
        ret, img = cap.read()
        if ret == True:
            cv2.imwrite('original video.png', img)
